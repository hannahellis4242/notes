...
```math
-\frac{\mu}{r^2}\hat{\mathbf{r}} \times \mathbf{h} = \frac{d}{dt}(\mathbf{v}\times\mathbf{h})
```
...
```math
\begin{align*}
\frac{d}{dt}(\mathbf{v}\times\mathbf{h}) &= -\frac{\mu}{r^2}(- r\left[ \dot{r}\hat{\mathbf{r}} +\mathbf{v}\right]) \\
&=\frac{\mu}{r}(\left[ \dot{r}\hat{\mathbf{r}} +\mathbf{v}\right])
\end{align*}
```
